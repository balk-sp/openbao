...
func oidcProviderPaths(i *IdentityStore) []*framework.Path {
	return []*framework.Path{
		...
		},
		{
			...
			HelpDescription: "The OIDC Introspect Endpoint allows a client to lookup the validity, and potentially the audience and expiration of an Access Token.",
		},
	}
}
...
func (i *IdentityStore) pathOIDCIntrospectForProvider(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	if err != nil {
		return i.convertIntrospectErrorToInactiveResponse("token lookup failed")
	}
	if tokenEntry == nil {
		return i.convertIntrospectErrorToInactiveResponse("non-existing or expired token")
	}
	if tokenEntry.Type != logical.TokenTypeBatch {
		return i.convertIntrospectErrorToInactiveResponse("token must be of type 'batch'")
	}
	...
	if !okClientID {
		return i.convertIntrospectErrorToInactiveResponse("missing client ID in token metadata")
	}
	...
	if err != nil {
		return i.convertIntrospectErrorToInactiveResponse("endpoint provider lookup failed")
	}
	if endpointProvider == nil {
		return i.convertIntrospectErrorToInactiveResponse("endpoint provider not found")
	}
	if !endpointProvider.allowedClientID(accessTokenClientID) {
		return i.convertIntrospectErrorToInactiveResponse("endpoint provider has not authorized client of token")
	}

	accessTokenClient, err := i.clientByID(ctx, req.Storage, accessTokenClientID)
	if err != nil {
		return i.convertIntrospectErrorToInactiveResponse("client lookup failed")
	}
	if accessTokenClient == nil {
		return i.convertIntrospectErrorToInactiveResponse("client of token not found")
	}
	accessTokenExp, err := tokenEntry.SentinelGet("expiration_time_unix")
	if err != nil {
		return i.convertIntrospectErrorToInactiveResponse("token expiration calculation failed")
	}
...
func (i *IdentityStore) convertIntrospectErrorToInactiveResponse(errorDescription string) (*logical.Response, error) {
